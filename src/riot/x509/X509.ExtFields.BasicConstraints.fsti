...
: Tot ( if isCA then
         ...
         ( asn1_value_int32_of_type SEQUENCE ))
= if isCA then
  ( len_of_x509_basicConstraints_extValue_payload_hasLen )
  else
  ( len_of_asn1_primitive_TLV false )

val lemma_serialize_x509_basicConstraints_extValue_payload_size
  (isCA: bool)
  (x: x509_basicConstraints_extValue_payload_t isCA)
: Lemma (
  ...
  ( if isCA then
    ( let x = x <: asn1_boolean_true_t `tuple2` datatype_of_asn1_type INTEGER in
      (v (len_of_x509_basicConstraints_extValue_payload true (snd x)  )))
    else
    ( v (len_of_x509_basicConstraints_extValue_payload false ) )))
...
noextract
val parse_x509_basicConstraints_extValue
  (isCA: bool)
: parser ((parse_asn1_envelop_tag_with_TLV_kind (SEQUENCE))) (x509_basicConstraints_extValue_t isCA)

noextract
val serialize_x509_basicConstraints_extValue
  (isCA: bool)
: serializer (parse_x509_basicConstraints_extValue isCA)

noextract inline_for_extraction
val serialize32_x509_basicConstraints_extValue_backwards
  (isCA: bool)
: serializer32_backwards (serialize_x509_basicConstraints_extValue isCA)

val lemma_serialize_x509_basicConstraints_extValue_unfold
  (isCA: bool)
...
val lemma_serialize_x509_basicConstraints_extValue_size
  (isCA: bool)
  (x: x509_basicConstraints_extValue_t isCA)
: Lemma (
...
: Tot ( if isCA then
         ( (datatype_of_asn1_type INTEGER)
           -> Tot (asn1_value_int32_of_type OCTET_STRING))
        else
        ( asn1_value_int32_of_type OCTET_STRING ))
= if isCA then
  ( fun (pathLen: datatype_of_asn1_type INTEGER) ->
      len_of_TLV
        (SEQUENCE)
...
val lemma_serialize_x509_basicConstraints_extValue_size_exact
  (isCA: bool)
  (x: x509_basicConstraints_extValue_t isCA)
: Lemma (
  ...
  ( let x = x <: asn1_boolean_true_t `tuple2` datatype_of_asn1_type INTEGER in
    ...
    == v (len_of_x509_basicConstraints_extValue true (snd x) ))
  else
  ( length_of_opaque_serialization (serialize_x509_basicConstraints_extValue isCA) x
...
val lemma_serialize_x509_basicConstraints_size
  (isCA: bool)
  (x: x509_basicConstraints_t isCA)
...
val lemma_serialize_x509_basicConstraints_size_exact
  (isCA: bool)
...
: Lemma (
  ...
  ( if isCA then
    ( let x = snd ext <: asn1_boolean_true_t `tuple2` datatype_of_asn1_type INTEGER in
      (v (len_of_x509_basicConstraints true (snd x))) )
    else
    ( (v (len_of_x509_basicConstraints false ) ))))
