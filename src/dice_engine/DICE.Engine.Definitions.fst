...
type dice_hash_alg = a:S.hash_alg {
  a =!= S.MD5 /\ a =!= S.SHA2_224 /\ a =!= S.Blake2S /\ a =!= S.Blake2B /\ a =!= S.SHA1 /\
  (not (S.is_shake a)) /\
  Some? (S.max_input_length a)
}
...
let dice_hash (alg:dice_hash_alg) : H.hash_st alg =
  match alg with
  | S.SHA2_256 -> Hacl.Streaming.SHA2.hash_256
  | S.SHA2_384 -> Hacl.Streaming.SHA2.hash_384
  | S.SHA2_512 -> Hacl.Streaming.SHA2.hash_512
...
unfold type hashable_len = i: size_t {0 < v i /\ v i <= (Some?.v (S.max_input_length alg))}
