...
= match alg with
  ...
  | AlgID_Ed25519    -> datatype_of_asn1_type OID
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( let x' = x'<: datatype_of_asn1_type OID in
                    x' = OID_ED25519 )
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( let x = x<: algorithmIdentifier_Ed25519_t in
                    x.algID_ed25519 )
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( parse_asn1_TLV_kind_of_type OID )
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( parse_asn1_TLV_of_type OID
                    `parse_filter`
                    filter_algorithmIdentifier_t' alg
                    `parse_synth`
                    synth_algorithmIdentifier_t alg)
...
: Lemma (
  ...
  | AlgID_Ed25519    -> ( serialize (serialize_algorithmIdentifier alg) x ==
...
= match alg with
  | AlgID_ECDSA_P256 -> ( serialize_nondep_then_eq
                    ...
                    (* prf*) ()
                    (* in *) x )
  | AlgID_Ed25519    -> ( serialize_synth_eq
                    (* p1 *) (parse_asn1_TLV_of_type OID
                              `parse_filter`
                              filter_algorithmIdentifier_t' alg)
                    (* f2 *) (synth_algorithmIdentifier_t alg)
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( lemma_serialize_algorithmIdentifier_unfold alg x;
...
= match alg with
  ...
  | AlgID_Ed25519    -> inbound_sequence_value_of (serialize_algorithmIdentifier alg)
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( serialize_asn1_sequence_TLV (serialize_algorithmIdentifier alg) )

let lemma_serialize_algorithmIdentifier_sequence_TLV_unfold
  (alg: supported_crypto_alg_t)
  (x: algorithmIdentifier_t_inbound alg)
: Lemma ( predicate_serialize_asn1_sequence_TLV_unfold (serialize_algorithmIdentifier alg) x )
...
let lemma_serialize_algorithmIdentifier_sequence_TLV_size
  (alg: supported_crypto_alg_t)
  (x: algorithmIdentifier_t_inbound alg)
: Lemma ( predicate_serialize_asn1_sequence_TLV_size (serialize_algorithmIdentifier alg) x )
= match alg with
...
: Lemma (
  ...
  | AlgID_Ed25519    -> ( lemma_serialize_algorithmIdentifier_size alg x;
...
= match alg with
  ...
  | AlgID_Ed25519    -> ( serialize32_asn1_sequence_TLV_backwards
                    (* s32 *) (serialize32_algorithmIdentifier_backwards alg) )
...
=
  match alg with
  | AlgID_Ed25519 -> ( let algID: algorithmIdentifier_t alg = { algID_ed25519 = OID_ED25519 } in
