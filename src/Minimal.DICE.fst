...
: HST.StackInline (l2: list t2{List.length l2 == I.v size})
  ...
  (ensures  fun h0 l2 h1 ->
    h0 == h1 /\
    B.live h1 b1 /\
    B.(Seq.equal (as_seq h0 b1) (as_seq h1 b1)) /\
    B.modifies B.loc_none h0 h1 /\
    I.v size == 0 ==>
        l2 == Nil /\
    I.v size =!= 0 ==>
...
#reset-options "--z3rlimit 50 --max_fuel 100 --max_ifuel 100"
assume val buffer_map_alloca
  (size: I.uint_32{I.v size > 0})
  (#t1 #t2: Type)
  (f: t1 -> t2)
  (b1: B.buffer t1{B.length b1 >= I.v size})
...
#reset-options "--z3rlimit 100 --max_fuel 100 --max_ifuel 100 --query_stats"
let dice_main
  (st: state)
