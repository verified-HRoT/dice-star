...
= match alg with
  ...
  | AlgID_Ed25519    -> (snd x).bs_len = 33ul &&
...
: Lemma ( predicate_serialize_asn1_sequence_TLV_unfold (serialize_subjectPublicKeyInfo alg) x )
...
let lemma_serialize_subjectPublicKeyInfo_sequence_TLV_size
  (alg: supported_crypto_alg_t)
  (x: subjectPublicKeyInfo_t_inbound alg)
: Lemma ( predicate_serialize_asn1_sequence_TLV_size (serialize_subjectPublicKeyInfo alg) x )
...
: Lemma (
  match alg with
  | AlgID_Ed25519   -> ( length_of_opaque_serialization (serialize_subjectPublicKeyInfo_sequence_TLV alg) x == 44 )
)
=
  match alg with
  | AlgID_Ed25519   -> ( (* reveal the SEQUENCE envelop *)
                   ...
                   lemma_serialize_subjectPublicKeyInfo_sequence_TLV_size   alg x;
                     ...
                     assert ( let bs: pubkey_t AlgID_Ed25519 = x.subjectPubKey in
