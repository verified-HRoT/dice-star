...
{
  uint32_t offset0 = serialize32_RDN_COUNTRY(x.aliasKeyTBS_issuer_Country, input, pos);
  uint32_t offset2 = offset0;
  uint32_t pos1 = pos - offset2;
  uint32_t offset = serialize32_RDN_ORGANIZATION(x.aliasKeyTBS_issuer_Organization, input, pos1);
  uint32_t offset21 = offset;
  uint32_t pos2 = pos1 - offset21;
  uint32_t offset1 = serialize32_RDN_COMMON_NAME(x.aliasKeyTBS_issuer_Common, input, pos2);
...
{
  uint32_t offset0 = serialize32_aliasKeyTBS_issuer_payload_backwards(x, b, pos);
